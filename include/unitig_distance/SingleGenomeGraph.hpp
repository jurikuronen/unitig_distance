...
public:
    ...
    std::pair<std::size_t, real_t> distance_to_start(std::size_t path_idx, std::size_t idx) const { return std::make_pair(m_paths[path_idx].start_node, m_paths[path_idx].distance_to_start(idx)); }
    std::pair<std::size_t, real_t> distance_to_end(std::size_t path_idx, std::size_t idx) const { return std::make_pair(m_paths[path_idx].end_node, m_paths[path_idx].distance_to_end(idx)); }
...
private:
    ...
    void dfs_add_neighbors_to_stack(const Graph& graph, const std::vector<bool>& visited, std::vector<std::tuple<std::size_t, std::size_t, real_t>>& stack, std::size_t node);
