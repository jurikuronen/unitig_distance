...
public:
    Program_options(int argc, char** argv) {
        ...
        // Required if --n_queries > 0.
        if ((arg_reader = find_arg_value(argv, argv_end, "-S", "--spydrpick-output-file"))) m_spydrpick_output_filename = std::string(arg_reader);
        ...
        if ((arg_reader = find_arg_value(argv, argv_end, "-n", "--n-queries"))) m_n_queries = std::stoll(arg_reader);
        if ((arg_reader = find_arg_name(argv, argv_end, "-1", "--queries-one-based"))) m_one_based = true;
    ...
    const std::string& spydrpick_output_filename() const { return m_spydrpick_output_filename; }
    ...
    const std::string out_cg_filename() const { return out_stem() + ".ud_cg_queries"; }
    const std::string out_fcg_filename() const { return out_stem() + ".ud_fcg_queries"; }
    const std::string out_sgg_min_filename() const { return out_stem() + ".ud_sgg_min_queries"; }
    const std::string out_sgg_max_filename() const { return out_stem() + ".ud_sgg_max_queries"; }
    const std::string out_sgg_mean_filename() const { return out_stem() + ".ud_sgg_mean_queries"; }
    const std::string out_sgg_counts_filename() const { return out_stem() + ".ud_sgg_counts"; }
...
private:
    ...
    std::string m_spydrpick_output_filename = "";
    ...
    bool all_required_arguments_provided() {
        ...
        if (n_queries() > 0 && !m_spydrpick_output_filename.size()) std::cout << "Missing queries filename.\n", ok = false;
        if (!ok) print_no_args();
        return ok;
    }
    ...
    void print_help() {
        std::vector<std::string> options{
            ...
            "", "",
            "Required argument if -n [ --n-queries ] is greater than 0.", "",
            "  -S [ --spydrpick-output-filename ] arg", "Path to SpydrPick's output file.",
            "", "",
            "Optional arguments.", "",
            ...
            "  -n [ --n-queries ] arg (=inf)", "Number of queries to read from SpydrPick's output.",
            "  -1 [ --queries-one-based ]", "Set if SpydrPick output had one-based numbering.",
            "  -r [ --run-sggs-only ]", "Don't calculate queries for the combined graph.",
    ...
    void print_arguments() {
        ...
        if (n_queries() > 0) push_back(arguments, "  --spydrpick-output-file", spydrpick_output_filename());
        ...
        push_back(arguments, "  --n-queries", n_queries() == INT_T_MAX ? "ALL" : std::to_string(n_queries()));
        push_back(arguments, "  --run-sggs-only", run_sggs_only() ? "TRUE" : "FALSE");
        if (n_queries() > 0) push_back(arguments, "  --queries-one-based", one_based() ? "TRUE" : "FALSE");
